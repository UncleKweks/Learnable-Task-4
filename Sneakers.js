...
const sneakerMenu = [
  new Sneaker("Adidas Originals Superstar", 95, 30),
  new Sneaker("Nike Air Force 1", 110, 15),
  new Sneaker("Puma RS-X", 100, 12),
  new Sneaker("New Balance 990v5", 180, 20),
  new Sneaker("Reebok Classic Leather", 90, 7),
  new Sneaker("Converse One Star Pro", 75, 23),
  new Sneaker("Vans Sk8-Hi Classic", 70, 18),
];

console.log(
  "========================= Welcome to Favs Sneakers Store! ============================"
);
console.log(
  "================================================================================"
);
...
sneakerMenu.forEach((sneaker, index) => {
  console.log(
    `${index + 1}. ${sneaker.name.padEnd(25)} $${sneaker.price}          ${
      sneaker.quantity
    }`
  );
});
...
console.log(
  "========================= What Sneaker Do you Want to Order Today?? ==============="
);
...
function processOrder(choice) {
  ...
  if (selectedSneaker.quantity > 0) {
    ...
    console.log(
      `Sorry, ${selectedSneaker.name} is out of stock. Please choose another sneaker.`
    );
    setTimeout(() => {
      displaySneakerMenu();
...
function askToContinueShopping() {
  rl.question("Do you want to order another sneaker? (yes/no): ", (answer) => {
    if (answer.toLowerCase() === "yes") {
      ...
      if (total > 0) {
        console.log(
          `Please proceed to payment. Your total bill is: $${total}. Pay to My Account Favour Nzeh \n Bank Number: 7047872574 \n Bank Name: Opay`
        );
...
function handleUserInput(choice) {
  ...
  if (
    !isNaN(numericChoice) &&
    numericChoice >= 1 &&
    numericChoice <= sneakerMenu.length
  ) {
